use iced::{Font, Point, Size, Vector, widget::pop::Key};

pub const ICON_FONT: Font = Font::with_name("dragwin");

pub const FILLED_RECTANGLE_ICON: char = '\u{0F0C8}';

pub const HOLLOW_RECTANGLE_ICON: char = '\u{0F096}';

pub const FILLED_ELLIPSE_ICON: char = '\u{0F111}';

pub const HOLLOW_ELLIPSE_ICON: char = '\u{0F1DB}';

pub const LINE_ICON: char = '\u{0F149}';

pub const ARROW_ICON: char = '\u{E800}';

pub const FREE_HAND_ICON: char = '\u{0e800}';

pub const HIGHLIGHTER_ICON: char = '\u{0e800}';

pub const TEXT_ICON: char = '\u{0e800}';
...
impl Tool {
    ...
    pub fn icon(&self) -> String {
        match self {
            ...
            Tool::Circle { filled: true, .. } => FILLED_ELLIPSE_ICON,
            Tool::Circle { filled: false, .. } => HOLLOW_ELLIPSE_ICON,
    ...
    pub fn update(&mut self, initial_pt: Point, final_pt: Point) {
        match self {
            ...
            Self::Circle { center, radius, .. } => {
                *center = Point::new(initial_pt.x, initial_pt.y);
                *radius = final_pt.x - initial_pt.x;
            }
            Self::FreeHand { points } => {
    ...
    pub fn scale(&mut self, scale_factor: f32) {
        match self {
            ...
            Tool::Circle { center, radius, .. } => {
                *center = Point::new(center.x * scale_factor, center.y * scale_factor);
                *radius = radius.to_owned() * scale_factor;
            }
    ...
    pub fn is_valid(&self) -> bool {
        match self {
            ...
            Self::Circle { radius, .. } => radius.to_owned() != 0. as f32,
